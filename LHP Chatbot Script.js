...
function processInput(message) {
    ...
    const keywords = doc.topics().out('array'); // Extract keywords from user input
    const lowerCaseMessage = message.toLowerCase();

    // Check for hidden information queries
    if (keywords.includes("criminal") || lowerCaseMessage.includes("los pollos hermanos")) {
    ...
    if (keywords.includes("fact")) {
        return responses["fun fact"];
    } else if (keywords.includes("saul")) {
